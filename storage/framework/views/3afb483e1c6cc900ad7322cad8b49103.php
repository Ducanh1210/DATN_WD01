...
<div class="top-navbar">
    <div class="top-navbar-left">
        <a href="<?php echo e(route('home')); ?>" class="btn-back" title="Quay lại bản đồ">
...
<div class="main-layout" id="profile-app-container">
    ...
    <div class="dashboard-sidebar">
        <div class="sidebar-user-section">
            <div class="avatar-container <?php echo e($user->equippedFrame ? ($user->equippedFrame->image_url ? 'avatar-frame-wrapper has-png-frame' : 'avatar-frame-wrapper ' . $user->equippedFrame->css_style) : ''); ?>" id="sidebarAvatarContainer" title="Nhấp để thay ảnh hoặc đổi khung avatar">
                <img src="<?php echo e($user->avatar_formatted_url); ?>" 
                     alt="Avatar" 
                     class="user-avatar-img"
                     id="profileAvatarPreview"
                     onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name=<?php echo e(urlencode($user->display_name ?? $user->username)); ?>&background=1e3a5f&color=fff';">
                <?php if($user->equippedFrame && $user->equippedFrame->image_url): ?>
                    <img src="<?php echo e(asset($user->equippedFrame->image_url)); ?>" class="avatar-frame-png-overlay">
                <?php endif; ?>
            ...
            <div class="sidebar-user-chips">
                <span class="user-role-badge">
                    <?php echo e($user->role === 'admin' ? 'Admin' : ($user->role === 'moderator' ? 'Mod' : 'Member')); ?>

                </span>
                <span class="sidebar-points-chip">
                    <?php echo e(number_format($user->points)); ?> pts
                </span>
            </div>
        </div>
        ...
        <div class="nav flex-column sidebar-menu-tabs" id="settings-tabs" role="tablist">
            ...
            <button class="nav-link" id="tab-itineraries-btn" data-bs-toggle="pill" data-bs-target="#tab-itineraries" type="button" role="tab" aria-selected="false">
                ...
                <span class="menu-count-badge" id="itinerariesCountBadge"><?php echo e(isset($itineraries) ? $itineraries->count() : 0); ?></span>
            </button>
            ...
            <button class="nav-link" id="tab-business-btn" data-bs-toggle="pill" data-bs-target="#tab-business" type="button" role="tab" aria-selected="false">
                ...
                <?php if(isset($businessProfile) && $businessProfile->status === 'pending'): ?>
        ...
        <div class="workspace-toolbar">
            ...
            <div class="workspace-actions">
                <?php if(isset($notifications) && $notifications->count() > 0): ?>
                    <?php $unreadCount = $notifications->whereNull('read_at')->count(); ?>
                    <button type="button" class="notification-toolbar-btn" data-bs-toggle="modal" data-bs-target="#notificationListModal">
                        <span>Thông báo</span>
                        <?php if($unreadCount > 0): ?>
                            <span class="notification-unread-badge"><?php echo e($unreadCount); ?></span>
                        <?php endif; ?>
                    </button>
                <?php endif; ?>
            </div>
        ...
        <div class="tab-content" id="settings-tabContent">
            ...
            <div class="tab-pane fade show active" id="tab-profile" role="tabpanel">
                        ...
                        <div class="info-list mb-0">
                            <div class="info-item">
                                ...
                                <div class="info-value" style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                                    <span id="profileTopDisplayName"><?php echo e($user->display_name ?? $user->username); ?></span>
                                ...
                                <div class="notification-modal-list">
                                    <?php if(isset($notifications) && $notifications->count() > 0): ?>
                                        <?php $__currentLoopData = $notifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $noti): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <div class="notification-modal-item <?php echo e(is_null($noti->read_at) ? 'is-unread' : ''); ?>">
                                                <div class="notification-modal-head">
                                                    <div class="notification-modal-title"><?php echo e($noti->title); ?></div>
                                                    <div class="notification-modal-time"><?php echo e($noti->created_at->format('d/m/Y H:i')); ?></div>
                                                </div>
                                                <div class="notification-modal-body"><?php echo e($noti->message ?: 'Không có nội dung chi tiết.'); ?></div>
                                            </div>
                                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                    <?php else: ?>
                ...
                <div class="profile-stat-grid">
                    <div class="stat-tile">
                        ...
                        <div class="stat-value"><?php echo e(number_format($user->points)); ?></div>
                        <div class="stat-hint">Điểm thưởng hiện có</div>
                    </div>
                    <div class="stat-tile">
                        <div class="stat-label">Địa điểm đã lưu</div>
                        <div class="stat-value"><?php echo e($favorites->count()); ?></div>
                        <div class="stat-hint">Favorites</div>
                    </div>
                    <div class="stat-tile">
                        <div class="stat-label">Lịch trình</div>
                        <div class="stat-value"><?php echo e(isset($itineraries) ? $itineraries->count() : 0); ?></div>
                        <div class="stat-hint">AI itineraries</div>
                    </div>
                    <div class="stat-tile">
                        <div class="stat-label">Nhận xét</div>
                        <div class="stat-value"><?php echo e($comments->count()); ?></div>
                    ...
                    <div class="content-panel">
                        <div class="panel-head">
                            ...
                            <span class="user-role-badge" style="background:#f1f5f9;color:#475569;border-color:#e2e8f0;">
                                <?php echo e($user->status === 'active' ? 'Active' : 'Locked'); ?>
            ...
            <div class="tab-pane fade" id="tab-points" role="tabpanel">
                <div class="profile-stat-grid" style="margin-bottom:12px;">
                    ...
                    <div class="stat-tile">
                        ...
                        <div class="stat-value"><?php echo e(number_format(count($pointHistory ?? []))); ?></div>
                        <div class="stat-hint">Sau khi gộp · <?php echo e(number_format($pointTxTotal ?? 0)); ?> bản ghi gốc</div>
                    </div>
                    <div class="stat-tile">
                ...
                <div class="content-panel">
                    ...
                    <div class="data-list" id="pointsHistoryList">
                        <?php $__empty_1 = true; $__currentLoopData = ($pointHistory ?? []); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <?php
                                $badgeMap = [
                            ...
                            <div class="data-row points-history-row"
                                 data-filter="<?php echo e($row['filter']); ?>">
                                <div style="min-width:0;">
                                    <div style="font-variant-numeric:tabular-nums;font-size:0.72rem;font-weight:700;color:#0f172a;">
                                        <?php echo e($row['created_at']->format('d/m')); ?>

                                    </div>
                                    <div style="font-variant-numeric:tabular-nums;font-size:0.68rem;color:#94a3b8;">
                                        <?php echo e($row['aggregated'] ? 'cả ngày' : $row['created_at']->format('H:i')); ?>

                                    </div>
                                </div>
                                <div style="min-width:0;">
                                    <div class="d-flex align-items-center gap-2 flex-wrap mb-1">
                                        <span class="badge bg-<?php echo e($badgeTone); ?> bg-opacity-10 text-<?php echo e($badgeTone); ?> border-0 px-2 py-1" style="font-size:0.68rem;border-radius:3px!important;"><?php echo e($badgeLabel); ?></span>
                                        <?php if(!empty($row['aggregated'])): ?>
                                            <span style="font-size:0.65rem;color:#64748b;"><?php echo e($row['count']); ?> lần · đã gộp</span>
                                        <?php endif; ?>
                                    </div>
                                ...
                                <div class="fw-bold <?php echo e($row['amount'] >= 0 ? 'text-success' : 'text-danger'); ?>"
                ...
                <div class="sec-split">
                    <div class="content-panel">
                        ...
                        <div class="ops-form-card">
                            ...
                            <form action="<?php echo e(route('client.profile.password')); ?>" method="POST">
                                <?php echo csrf_field(); ?>
                        ...
                        <div class="checklist">
                            <div class="checklist-item">
                                <div>
                                    ...
                                    <div class="ci-value"><?php echo e($user->email); ?></div>
                                </div>
                            ...
                            <div class="checklist-item">
                                <div>
                                    ...
                                    <div class="ci-value"><?php echo e(!empty($user->password_hash) ? 'Đã thiết lập' : 'Chưa thiết lập'); ?></div>
                                </div>
                                <?php if(!empty($user->password_hash)): ?>
                                    <span class="ci-ok">OK</span>
                                <?php else: ?>
                            ...
                            <div class="checklist-item">
                                <div>
                                    ...
                                    <div class="ci-value"><?php echo e($user->status === 'active' ? 'Đang hoạt động' : 'Bị khóa'); ?></div>
                                </div>
                                <span class="<?php echo e($user->status === 'active' ? 'ci-ok' : 'ci-warn'); ?>"><?php echo e($user->status === 'active' ? 'Active' : 'Locked'); ?></span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="tab-pane fade" id="tab-favorites" role="tabpanel">
                <div class="content-panel">
                    <div class="section-title">
                        <span>Địa điểm đã lưu</span>
                        <span class="dt-meta"><?php echo e($favorites->count()); ?> mục</span>
                    </div>
                    <div class="data-toolbar">
                        ...
                        <a href="<?php echo e(route('home')); ?>" class="btn-solid" style="text-decoration:none;">+ Thêm từ bản đồ</a>
                    </div>
                    <div class="data-list" id="favoritesGrid">
                        <?php $__empty_1 = true; $__currentLoopData = $favorites; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $location): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <div class="data-row favorite-card-wrapper" id="fav-card-<?php echo e($location->id); ?>">
                                <img src="<?php echo e($location->thumbnail_url); ?>" alt="<?php echo e($location->name); ?>" class="data-thumb" onerror="this.style.opacity='0.35'">
                                <div class="data-main">
                                    <div class="data-title"><?php echo e($location->name); ?></div>
                                    <p class="data-desc"><?php echo e($location->short_description ?? 'Không có mô tả.'); ?></p>
                                </div>
                                <div class="data-actions">
                                    <a href="<?php echo e(url('/#loc-' . $location->id)); ?>" class="btn-ghost">Bản đồ</a>
                                    <a href="<?php echo e(route('client.locations.360', $location->slug)); ?>" class="btn-solid">360°</a>
                                    <button type="button" class="btn-danger-ghost favorite-toggle-btn" data-location-id="<?php echo e($location->id); ?>">Bỏ thích</button>
                                </div>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                            <div id="noFavoritesMsg" class="empty-state" style="border:0;border-radius:0;">
                                <p>Bạn chưa lưu địa điểm nào.</p>
                                <a href="<?php echo e(route('home')); ?>" class="btn-solid" style="text-decoration:none;display:inline-block;">Tìm địa điểm trên bản đồ</a>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="tab-pane fade" id="tab-itineraries" role="tabpanel">
                <div class="content-panel">
                    <div class="section-title">
                        <span>Lịch trình đã lưu</span>
                        <span class="dt-meta"><?php echo e(isset($itineraries) ? $itineraries->count() : 0); ?> mục</span>
                    </div>
                    <div class="data-toolbar">
                        <div class="dt-left">
                        ...
                        <a href="<?php echo e(route('home')); ?>" class="btn-solid" style="text-decoration:none;" onclick="if(window.openTripPlanner){event.preventDefault(); window.openTripPlanner(true);}">+ Tạo lịch trình</a>
                    </div>
                    <div class="data-list" id="itinerariesGrid">
                        <?php $__empty_1 = true; $__currentLoopData = ($itineraries ?? collect()); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $it): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <div class="data-row itinerary itinerary-card-wrapper" id="itinerary-card-<?php echo e($it->id); ?>">
                                <div class="data-main">
                                    <div class="data-title"><?php echo e($it->title); ?></div>
                                    <p class="data-desc"><?php echo e($it->summary ?? $it->description ?? 'Không có tóm tắt.'); ?></p>
                                    <div class="it-card-meta" style="margin:8px 0 0;">
                                        <span class="it-card-chip"><?php echo e($it->total_days); ?> ngày</span>
                                        <?php if($it->estimated_cost): ?>
                                            <span class="it-card-chip"><?php echo e($it->estimated_cost); ?></span>
                                        <?php endif; ?>
                                        <span class="it-card-chip"><?php echo e($it->created_at?->format('d/m/Y')); ?></span>
                                    </div>
                                </div>
                                <div class="data-actions">
                                    <a href="<?php echo e(route('home')); ?>?itinerary=<?php echo e($it->id); ?>" class="btn-solid itinerary-view-btn" style="text-decoration:none;">Xem lại</a>
                                    <button type="button" class="btn-danger-ghost itinerary-delete-btn" data-id="<?php echo e($it->id); ?>">Xóa</button>
                                </div>
                            </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                            <div id="noItinerariesMsg" class="empty-state" style="border:0;border-radius:0;">
                                <p>Bạn chưa lưu lịch trình nào.</p>
                                <a href="<?php echo e(route('home')); ?>" class="btn-solid" style="text-decoration:none;display:inline-block;" onclick="if(window.openTripPlanner){event.preventDefault(); window.openTripPlanner(true);}">Lên lịch trình AI</a>
                            </div>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="tab-pane fade" id="tab-comments" role="tabpanel">
                <div class="content-panel">
                    <div class="section-title">
                        <span>Nhận xét của tôi</span>
                        <span class="dt-meta"><?php echo e($comments->count()); ?> mục</span>
                    </div>
                    <div class="data-toolbar">
                        <div class="dt-left">
                            <span class="filter-chip on">Tất cả</span>
                            ...
                            <tbody>
                                <?php $__empty_1 = true; $__currentLoopData = $comments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $comment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                                    <tr id="comment-row-<?php echo e($comment->id); ?>">
                                        <td>
                                            <a href="<?php echo e(url('/#loc-' . $comment->location_id)); ?>" class="text-decoration-none fw-semibold" style="color:var(--text-main);font-size:0.8rem;">
                                                <?php echo e($comment->location->name ?? 'Địa điểm ẩn'); ?>

                                            </a>
                                        </td>
                                        <td class="text-secondary" style="font-size:0.78rem;"><?php echo e($comment->content); ?></td>
                                        <td><span class="text-warning fw-bold" style="font-variant-numeric:tabular-nums;"><?php echo e($comment->rating ?? 0); ?> ★</span></td>
                                        <td style="font-size:0.72rem;font-variant-numeric:tabular-nums;white-space:nowrap;"><?php echo e($comment->created_at?->format('d/m/Y H:i')); ?></td>
                                        <td style="text-align:right;">
                                            <button type="button" class="btn-danger-ghost delete-comment-btn" data-comment-id="<?php echo e($comment->id); ?>">Xóa</button>
                                        </td>
                                    </tr>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                                    <tr id="noCommentsRow">
                                        ...
                                        <div class="biz-pending-card-body">
                                            <div class="biz-pending-fields">
                                                <div class="biz-pending-field">
                                                    ...
                                                    <div class="biz-pending-value is-primary"><?php echo e($businessProfile->business_name); ?></div>
                                                </div>
                                                <div class="biz-pending-field">
                                                    <span class="biz-pending-label">Danh mục</span>
                                                    <div class="biz-pending-value"><?php echo e($businessProfile->category ? $businessProfile->category->name : 'N/A'); ?></div>
                                                </div>
                                                <div class="biz-pending-field">
                                                    <span class="biz-pending-label">Địa chỉ</span>
                                                    <div class="biz-pending-value"><?php echo e($businessProfile->address_street); ?>, <?php echo e($businessProfile->address_city); ?>, <?php echo e($businessProfile->address_province); ?></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div class="content-panel">
                                ...
                                <div class="panel-note danger">
                                    <div>
                                        ...
                                        Hồ sơ “<?php echo e($businessProfile->business_name); ?>” không còn trên bản đồ. Bạn có thể đăng ký lại nếu cần.
                                    </div>
                                </div>
                                <a href="<?php echo e(route('client.profile.business.upgrade')); ?>" class="btn-solid" style="text-decoration:none;display:inline-block;">Đăng ký lại</a>
                            </div>
                        <?php else: ?>
                        <div class="content-panel">
                            ...
                            <div class="biz-feature-grid">
                                <div class="biz-feature">
                                    ...
                                    <a href="<?php echo e(route('client.locations.360', $loc->slug)); ?>" target="_blank" class="btn-ghost" style="display:inline-block;">Xem chi tiết</a>
                                </div>
                                <div class="biz-feature">
                                    <strong>Trang quản trị</strong>
                                    <p class="mb-2">Dashboard dành cho chủ doanh nghiệp.</p>
                                    <a href="<?php echo e(route('business.dashboard')); ?>" class="btn-solid" style="display:inline-block;text-decoration:none;">Vào quản trị</a>
                                </div>
                                <div class="biz-feature">
                                    <strong>Dịch vụ tour 360</strong>
                                    <p class="mb-2">Quay & dựng panorama theo nhu cầu, báo giá sau tư vấn.</p>
                                    <a href="<?php echo e(route('client.pano_service')); ?>" class="btn-ghost" style="display:inline-block;">Tìm hiểu & gửi yêu cầu</a>
                                </div>
                            </div>
                        </div>
                        <?php endif; ?>
                            ...
                            <div class="content-panel">
                                ...
                                <div class="biz-hero">
                                    <div>
                                        ...
                                        <a href="<?php echo e(route('client.profile.business.upgrade')); ?>" class="btn-solid" style="text-decoration:none;display:inline-block;padding:9px 16px;">Bắt đầu đăng ký</a>
                                    </div>
                                    <div class="biz-feature-grid">
                                        <div class="biz-feature">
                                ...
                                <p class="biz-pano-note">
                                    ...
                                    <a href="<?php echo e(route('client.pano_service')); ?>" target="_blank" rel="noopener">Tìm hiểu dịch vụ chụp Tour 360°</a>
                                </p>
                            </div>
                        <?php else: ?>
                            <div class="content-panel">
                                ...
                                <div class="panel-note danger">
                                    <div>
                                        ...
                                        Lý do: <?php echo e($businessProfile->reject_reason ?? 'Thông tin cung cấp chưa chính xác hoặc không đủ điều kiện.'); ?>

                                    </div>
                                </div>
                                <a href="<?php echo e(route('client.profile.business.upgrade')); ?>" class="btn-danger-ghost" style="text-decoration:none;display:inline-block;">Đăng ký lại</a>
                            </div>
                        <?php endif; ?>
                    <?php endif; ?>
                <?php else: ?>
                ...
                <div class="tab-content" id="avatarModalTabContent">
                    ...
                    <div class="tab-pane fade show active text-center" id="pane-avatar-photo" role="tabpanel">
                        <div class="mb-4 d-flex justify-content-center position-relative">
                            <div class="avatar-frame-wrapper <?php echo e($user->equippedFrame ? ($user->equippedFrame->image_url ? 'has-png-frame' : $user->equippedFrame->css_style) : ''); ?>" style="width: 200px; height: 200px;">
                                <img src="<?php echo e($user->avatar_formatted_url); ?>" 
                                     alt="Avatar" 
                                     id="avatarModalLargePreview" 
                                     onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name=<?php echo e(urlencode($user->display_name ?? $user->username)); ?>&background=0072FF&color=fff';"
                    ...
                    <div class="tab-pane fade" id="pane-avatar-frames" role="tabpanel">
                        ...
                        <div class="row g-3 style-scroll" style="max-height: 380px; overflow-y: auto; padding-right: 4px;">
                            <?php $__currentLoopData = $allFrames; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $frame): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php
                                ...
                                <div class="col-6 col-sm-4">
                                    <div class="p-3 border rounded-3 text-center h-100 d-flex flex-column justify-content-between bg-white position-relative <?php echo e($isEquipped ? 'border-primary shadow-sm' : ''); ?>">
                                        <?php if($isEquipped): ?>
                                        ...
                                        <div>
                                            <div class="avatar-frame-wrapper <?php echo e($frame->image_url ? 'has-png-frame' : $frame->css_style); ?> mx-auto my-2" style="width: 68px; height: 68px;">
                                                <img src="<?php echo e($user->avatar_formatted_url); ?>" onerror="this.onerror=null; this.src='https://ui-avatars.com/api/?name=<?php echo e(urlencode($user->display_name ?? $user->username)); ?>&background=0072FF&color=fff';" style="width: 100%; height: 100%; border-radius: 50%; object-fit: cover;">
                                                <?php if($frame->image_url): ?>
                                                    <img src="<?php echo e(asset($frame->image_url)); ?>" class="avatar-frame-png-overlay">
                                                <?php endif; ?>
                                            </div>
                                            <h6 class="fw-bold text-dark mb-2 text-truncate" style="font-size: 0.85rem;" title="<?php echo e($frame->name); ?>"><?php echo e($frame->name); ?></h6>
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <?php elseif($isUnlocked): ?>
                                                <button type="button" class="btn btn-primary btn-sm w-100 fw-medium rounded-2 btn-equip-frame" data-id="<?php echo e($frame->id); ?>" style="font-size: 0.775rem;">
                                                    Trang bị
                                                </button>
                                            <?php else: ?>
                                                <button type="button" 
                                                        class="btn btn-light text-muted btn-sm w-100 fw-medium rounded-2 border btn-lock-info" 
                                                        data-name="<?php echo e($frame->name); ?>"
                                                        data-desc="<?php echo e($frame->description); ?>"
                                                        data-style="<?php echo e($frame->css_style); ?>"
                                                        data-image="<?php echo e($frame->image_url ? asset($frame->image_url) : ''); ?>"
                                                        data-condition="<?php echo e($conditionText); ?>"
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        <?php if(isset($businessProfile) && $businessProfile->lat && $businessProfile->lng): ?>
                ...
                if (!pendingBizMap) {
                    ...
                    marker.bindPopup(`
                        <div style="font-family: 'Be Vietnam Pro', sans-serif; font-size: 0.85rem; padding: 2px;">
                            <strong style="color: #1e3a5f;"><?php echo e($businessProfile->business_name); ?></strong><br>
                            <span style="color: #64748b; font-size: 0.775rem;"><?php echo e($businessProfile->address_street); ?>, <?php echo e($businessProfile->address_city); ?></span>
                        </div>
                    `).openPopup();
                }
...
<?php /**PATH D:\laragon\www\DuAn_TN-main\resources\views/client/profile.blade.php ENDPATH**/ ?>