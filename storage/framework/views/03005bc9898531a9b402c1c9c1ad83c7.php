...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let tpMiniMap = null;
    let tpRouteMarkers = [];
    const IS_AUTHENTICATED = <?php echo json_encode(auth()->check(), 15, 512) ?>;
    const CURRENT_USER_ID = <?php echo json_encode(auth()->id(), 15, 512) ?>;
    const LOGIN_URL = <?php echo json_encode(route('login'), 15, 512) ?>;
    const PROFILE_URL = <?php echo json_encode(route('client.profile'), 15, 512) ?>;
    ...
    const TP_BOUNDARY_URL = <?php echo json_encode(asset('geo/ha-nam-old.geojson'), 15, 512) ?>;
    ...
    function startGeneration() {
        ...
        fetch('<?php echo e(route("client.trip_planner.generate")); ?>', {
            method: 'POST',
            headers: {
    ...
    if (saveBtn) {
        saveBtn.addEventListener('click', () => {
            ...
            fetch('<?php echo e(route("client.trip_planner.save")); ?>', {
                method: 'POST',
    ...
    (function bootSavedItineraryFromUrl() {
        ...
        if (!id) return;
        fetch('<?php echo e(url("/trip-planner")); ?>/' + encodeURIComponent(id), {
...
<?php /**PATH D:\laragon\www\datnv2\DuAn_TN\resources\views/components/trip-planner-widget.blade.php ENDPATH**/ ?>