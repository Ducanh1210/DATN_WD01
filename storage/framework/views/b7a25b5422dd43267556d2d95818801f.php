...
<div class="tab-content" id="locationTabsContent">
    ...
    <div class="tab-pane fade show active" id="info" role="tabpanel">
        <div class="card-minimal p-4 mb-4">
            <form action="<?php echo e(route('admin.locations.update', [$location->id] + request()->query())); ?>" method="POST" enctype="multipart/form-data" novalidate>
                <?php echo csrf_field(); ?>
...
unset($__errorArgs, $__bag); ?>" id="name" name="name" value="<?php echo e(old('name', $location->name)); ?>" style="border-color: #e2e8f0;">
...
unset($__errorArgs, $__bag); ?>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control form-control-sm" id="short_description" name="short_description" rows="3" style="border-color: #e2e8f0;"><?php echo e(old('short_description', $location->short_description)); ?></textarea>
                        </div>

                        <div class="mb-3">
                            <label for="address" class="form-label text-dark fw-medium" style="font-size: 0.825rem;">Địa chỉ</label>
                            <input type="text" class="form-control form-control-sm" id="address" name="address" value="<?php echo e(old('address', $location->address)); ?>" style="border-color: #e2e8f0;">
                        </div>
                    </div>
...
unset($__errorArgs, $__bag); ?>" id="lat" name="lat" value="<?php echo e(old('lat', $location->lat)); ?>" style="border-color: #e2e8f0;">
                                <?php $__errorArgs = ['lat'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
...
unset($__errorArgs, $__bag); ?>" id="lng" name="lng" value="<?php echo e(old('lng', $location->lng)); ?>" style="border-color: #e2e8f0;">
                                <?php $__errorArgs = ['lng'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
...
unset($__errorArgs, $__bag); ?>
                            <?php if($location->thumbnail_url): ?>
                                <div class="mt-2">
                                    ...
                                    <img src="<?php echo e(asset('storage/' . $location->thumbnail_url)); ?>" class="rounded border" style="max-height: 90px; object-fit: cover;" alt="Thumbnail">
                                </div>
                            <?php endif; ?>
                        </div>
                    </div>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4 pt-3 border-top" style="border-color: var(--border-light) !important;">
                    <a href="<?php echo e(route('admin.locations.index', request()->query())); ?>" class="btn-minimal text-decoration-none">Quay lại</a>
                    <button type="submit" class="btn-minimal btn-minimal-primary">Cập nhật Thông tin</button>
                </div>
            </form>
        </div>

        <?php if(!empty($isBusinessLocation)): ?>
            <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-3 px-1">
                <span class="text-muted" style="font-size: 0.78rem;">
                    Chủ DN: <?php echo e($businessOwner->email ?? ('#' . $location->created_by)); ?>
            ...
            <div class="modal fade" id="revokeBizModal" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <form method="POST" action="<?php echo e(route('admin.locations.revoke_business', $location->id)); ?>" id="revokeBizForm">
                            <?php echo csrf_field(); ?>
                            <?php $__currentLoopData = request()->only(['search', 'category_id', 'sort_dir']); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $qKey => $qVal): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <input type="hidden" name="<?php echo e($qKey); ?>" value="<?php echo e($qVal); ?>">
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            ...
                            <div class="modal-body">
                                <p class="text-secondary mb-2" style="font-size: 0.82rem;">
                                    Gỡ chủ DN khỏi <strong><?php echo e($location->name); ?></strong> (địa điểm vẫn trên map).
                                </p>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-3 col-sm-4 image-card" id="img-<?php echo e($img->id); ?>">
                            <div class="card-minimal h-100 position-relative p-1">
                                <img src="<?php echo e(Storage::url($img->image_url)); ?>" class="rounded w-100 object-fit-cover" height="130" alt="">
                                <div class="position-absolute top-0 end-0 p-2">
                                    <button class="btn-minimal py-1 px-2 text-danger bg-white shadow-sm border btn-delete-image" data-id="<?php echo e($img->id); ?>" style="font-size: 0.75rem;">Xóa</button>
                                </div>
                            </div>
                        </div>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    ...
    <div class="tab-pane fade" id="pano" role="tabpanel">
        <div class="card-minimal text-center p-5 mb-4">
            ...
            <a href="<?php echo e(route('admin.locations.360_editor', $location->id)); ?>" class="btn-minimal btn-minimal-primary mx-auto text-decoration-none px-4 py-2" style="font-size: 0.85rem;">
                Mở 360 Tour Editor
            </a>
        </div>
    </div>
    ...
    <div class="tab-pane fade" id="audio" role="tabpanel">
        ...
        <?php if($location->audio_url): ?>
            <div class="card-minimal p-3 mb-4">
                ...
                <div>
                    <div class="d-flex align-items-center gap-3 p-3 bg-light rounded border <?php echo e(isset($location->attributes['tts_text']) ? 'mb-3' : ''); ?>" style="border-color: #e2e8f0 !important;">
                    ...
                    <?php if(isset($location->attributes['tts_text'])): ?>
                        <div class="p-3 bg-light rounded border" style="border-color: #e2e8f0 !important;">
                            ...
                            <div class="text-dark bg-white p-3 rounded border" style="font-size: 0.8rem; line-height: 1.6; border-left: 3px solid #cbd5e1 !important; white-space: pre-wrap;">"<?php echo e($location->attributes['tts_text']); ?>"</div>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
                ...
                <div class="card-minimal p-3 h-100 d-flex flex-column justify-content-between">
                    ...
                    <div>
                        ...
                        <button type="button" class="btn-minimal btn-minimal-primary w-100 py-2 text-center" onclick="document.getElementById('audioUploadInput').click()">
                            <?php echo e($location->audio_url ? 'Đổi file audio khác' : 'Chọn file từ thiết bị'); ?>
                ...
                <div class="card-minimal p-3 h-100">
                    ...
                    <div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="ttsTextInput" class="form-control form-control-sm" rows="4" placeholder="Nhập đoạn thuyết minh giới thiệu tại đây (Tối đa 5000 ký tự)..." style="border-color: #e2e8f0;"><?php echo e(old('tts_text', $location->attributes['tts_text'] ?? $location->short_description)); ?></textarea>
...
<script>
    ...
    function handleImageFiles(files) {
        ...
        for(let i=0; i<files.length; i++) {
            ...
            $.ajax({
                url: '<?php echo e(route('admin.locations.upload_image', $location->id, false)); ?>',
                type: 'POST',
                data: formData,
                contentType: false,
                processData: false,
    ...
    $('#audioUploadInput').change(function() {
        ...
        $.ajax({
            url: '<?php echo e(route("admin.locations.upload_audio", $location->id, false)); ?>',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $(document).on('click', '#btnDeleteAudio', function() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?php echo e(route("admin.locations.delete_audio", $location->id, false)); ?>',
                    type: 'DELETE',
                    success: function(res) {
                        if (res.success) {
                            Swal.fire({
    ...
    $('#btnGenerateTts').click(function() {
        ...
        $.ajax({
            url: '<?php echo e(route("admin.locations.generate_tts", $location->id, false)); ?>',
            type: 'POST',
            data: {
                text: text,
                voice_id: voiceId,
...
<?php echo $__env->make('admin.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\laragon\www\DuAn_TN-main\resources\views/admin/locations/edit.blade.php ENDPATH**/ ?>