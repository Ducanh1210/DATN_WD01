...
<?php $__env->startSection('actions'); ?>
    <?php if (! ($trash)): ?>
        <a href="<?php echo e(route('admin.locations.create', request()->query())); ?>" class="btn-minimal btn-minimal-primary">Thêm địa điểm</a>
    <?php endif; ?>
<?php $__env->stopSection(); ?>

<?php $__env->startSection('content'); ?>
<div class="d-flex align-items-center justify-content-between gap-2 mb-3">
    <a href="<?php echo e(route('admin.locations.index', request()->except(['trash', 'page']))); ?>"
       class="btn-minimal py-1 px-3 text-decoration-none <?php echo e(!$trash ? 'btn-minimal-primary' : ''); ?>"
       style="font-size:0.78rem;<?php echo e(!$trash ? 'color:#fff;' : ''); ?>">
        Đang hoạt động (<?php echo e($activeCount); ?>)
    </a>
    <a href="<?php echo e(route('admin.locations.index', array_merge(request()->except(['page']), ['trash' => 1]))); ?>"
       class="btn-minimal py-1 px-3 text-decoration-none <?php echo e($trash ? 'btn-minimal-primary' : ''); ?>"
       style="font-size:0.78rem;<?php echo e($trash ? 'color:#fff;' : ''); ?>">
        Thùng rác (<?php echo e($trashedCount); ?>)
    </a>
</div>

<!-- Form Lọc & Tìm kiếm Minimalist -->
<div class="card-minimal mb-3 p-3">
    <form action="<?php echo e(route('admin.locations.index')); ?>" method="GET" class="row g-2 align-items-center">
        <?php if($trash): ?>
            <input type="hidden" name="trash" value="1">
        <?php endif; ?>
        <div class="col-md-7">
            <input type="text" name="search" class="form-control form-control-sm" placeholder="Nhập tên địa điểm cần tìm..." value="<?php echo e(request('search')); ?>" style="border-color: #e2e8f0;">
        </div>
        <div class="col-md-5">
            ...
            <thead>
                <tr>
                    <th class="text-center" style="width: 60px;">
                        <a href="<?php echo e(request()->fullUrlWithQuery(['sort_dir' => $sortDir === 'desc' ? 'asc' : 'desc', 'page' => null])); ?>" class="text-muted text-decoration-none">
                            ID <?php echo e($sortDir === 'asc' ? '↑' : '↓'); ?>
                    ...
                    <th>Tên Địa điểm</th>
                    <th>Danh mục</th>
                    <th><?php echo e($trash ? 'Ngày gỡ' : 'Tọa độ'); ?></th>
                    ...
                    <tr>
                        ...
                        <td>
                            <?php if($item->thumbnail_url): ?>
                                <img src="<?php echo e(asset('storage/' . $item->thumbnail_url)); ?>" class="rounded" style="width: 48px; height: 32px; object-fit: cover;" alt="<?php echo e($item->name); ?>">
                            <?php else: ?>
                        ...
                        <?php $isBizLoc = $item->created_by && in_array($item->created_by, $businessOwnerIds); ?>
                        <td>
                            ...
                            <div class="text-muted text-truncate" style="max-width: 260px; font-size: 0.725rem;"><?php echo e($item->address ?? 'Ninh Bình'); ?></div>
                        </td>
                        <td>
                        ...
                        <td>
                            <?php if($trash): ?>
                                <span class="text-muted" style="font-size: 0.75rem;"><?php echo e(optional($item->deleted_at)->format('d/m/Y H:i')); ?></span>
                            <?php else: ?>
                                <span class="text-muted" style="font-size: 0.75rem;"><?php echo e($item->lat); ?>, <?php echo e($item->lng); ?></span>
                            <?php endif; ?>
                        </td>
                        <td class="text-center">
                            ...
                            <?php else: ?>
                                <span class="badge-minimal"><?php echo e(ucfirst($item->status)); ?></span>
                            <?php endif; ?>
                        </td>
                        <td class="text-end pe-4">
                            <?php if($trash): ?>
                                <form action="<?php echo e(route('admin.locations.restore', $item->id)); ?>" method="POST" class="d-inline">
                                    <?php echo csrf_field(); ?>
                                ...
                                <?php if($isBizLoc): ?>
                                    <button type="button" class="btn-minimal py-1 px-2 text-danger btn-force-delete-biz" style="font-size: 0.75rem;"
                                        data-action="<?php echo e(route('admin.locations.force_destroy', $item->id)); ?>"
                                        data-name="<?php echo e($item->name); ?>">Xóa vĩnh viễn</button>
                                <?php else: ?>
                                    <form action="<?php echo e(route('admin.locations.force_destroy', $item->id)); ?>" method="POST" class="d-inline"
                                ...
                                <?php endif; ?>
                            <?php else: ?>
                                <a href="<?php echo e(route('admin.locations.edit', [$item->id] + request()->query())); ?>" class="btn-minimal py-1 px-2 text-decoration-none me-1" style="font-size: 0.75rem;">Sửa</a>
                                <?php if($isBizLoc): ?>
                                    <button type="button" class="btn-minimal py-1 px-2 text-danger btn-delete-biz" style="font-size: 0.75rem;"
                                        data-action="<?php echo e(route('admin.locations.destroy', $item->id)); ?>"
                                        data-name="<?php echo e($item->name); ?>">Xóa</button>
                                <?php else: ?>
                                    <form action="<?php echo e(route('admin.locations.destroy', $item->id)); ?>" method="POST" class="d-inline"
                ...
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_1): ?>
                    <tr>
                        <td colspan="7" class="text-center text-muted py-4">
                            <?php echo e($trash ? 'Thùng rác trống.' : 'Chưa có địa điểm nào.'); ?>
...
<?php echo $__env->make('admin.layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\laragon\www\DuAn_TN-main\resources\views/admin/locations/index.blade.php ENDPATH**/ ?>